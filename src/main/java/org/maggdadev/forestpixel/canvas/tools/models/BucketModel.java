...
public class BucketModel extends ToolModel {
    ...
    public void applyToCanvas(CanvasModel canvasModel, CanvasContext canvasContext, int xIdx, int yIdx) {
        ...
        while(!stack.isEmpty()) {
            ...
            while(x1 <= x2) {
                ...
                x1 ++;
                while(x1 < x2 && (!isToBeFilled(x1, y, startColor, canvasModel))) {
                    x1 ++;
                }
                x = x1;
            }
    ...
    private boolean isToBeFilled(int x, int y, Color startColor, CanvasModel canvasModel) {
        return canvasModel.isOnCanvas(x,y) && startColor.equals(canvasModel.getPixelColor(x,y));
    }
